...
struct Emitter {
    ...
    const Options& opt;
    std::string out;
    bool external_linkage = false; // multi-file mode: functions are non-static
    ...
    void emit_function(const analysis::Function& fn) {
        ...
        // In multi-file mode functions have external linkage (declared in the header);
        // in single-file mode they are static (file-local).
        out += external_linkage ? "void " + name + "([[maybe_unused]] EEContext& ctx) {\n"
                                : "static void " + name + "([[maybe_unused]] EEContext& ctx) {\n";
...
std::string emit_module(const elf::Image& image, const analysis::Result& res, const Config& cfg,
                        const Options& opt) {
    Emitter e{image, res, cfg, opt, {}, false};
...
MultiFileResult emit_module_multi(const elf::Image& image, const analysis::Result& res,
    ...
    const std::string base = opt.output_base.empty() ? "recomp" : opt.output_base;
    const std::string header_name = base + ".recomp.h";
    const size_t per_file = opt.functions_per_file > 0 ? opt.functions_per_file : 500;
    const size_t n = res.functions.size();

    // --- shared header: forward declarations + register_functions declaration ---
    std::string& hdr = result.header;
    hdr += "// Auto-generated by ee-recomp (EERecomp). Do not edit.\n";
    hdr += "#pragma once\n";
    hdr += "#include \"ee/runtime.hpp\"\n\n";
    hdr += "using ee::rt::EEContext;\n\n";
    hdr += "// Forward declarations (external linkage; defined in the .recomp.*.cpp files).\n";
    for (const analysis::Function& fn : res.functions)
        hdr += "void " + fname(fn.start) + "([[maybe_unused]] EEContext& ctx);\n";
    hdr += "\n// Registers all recompiled functions with the runtime.\n";
    hdr += "void register_functions(ee::rt::Runtime& rt);\n";

    // --- .cpp files: each implements a chunk of functions ---
    const size_t num_chunks = (n + per_file - 1) / per_file;
    for (size_t chunk = 0; chunk < num_chunks; ++chunk) {
        const size_t begin = chunk * per_file;
        const size_t end = std::min(begin + per_file, n);

        Emitter e{image, res, cfg, opt, {}, true}; // external_linkage = true
        char filebuf[256];
        std::snprintf(filebuf, sizeof filebuf, "%s.recomp.%zu.cpp", base.c_str(), chunk);
        e.raw("// Auto-generated by ee-recomp (EERecomp). Do not edit.");
        {
            char cmt[128];
            std::snprintf(cmt, sizeof cmt, "// Chunk %zu / %zu (functions %zu-%zu)",
                          chunk, num_chunks - 1, begin, end - 1);
            e.raw(cmt);
        }
        e.raw("#include <cmath>");
        e.raw((std::string("#include \"") + header_name + "\"").c_str());
        e.raw("");
        e.raw("using namespace ee;");
        e.raw("using namespace ee::rt;");
        e.raw("");
        for (size_t i = begin; i < end; ++i)
            e.emit_function(res.functions[i]);

        result.files.emplace_back(filebuf, e.out);
    }

    // --- register.cpp: register_functions implementation ---
    {
        std::string reg;
        reg += "// Auto-generated by ee-recomp (EERecomp). Do not edit.\n";
        reg += "#include \"" + header_name + "\"\n\n";
        reg += "void register_functions(ee::rt::Runtime& rt) {\n";
        char line[128];
        for (const analysis::Function& fn : res.functions) {
            std::snprintf(line, sizeof line, "    rt.add(0x%08Xu, %s);\n",
                          fn.start, fname(fn.start).c_str());
            reg += line;
        }
        reg += "}\n";
        result.files.emplace_back(base + ".recomp.register.cpp", reg);
    }
